<template>
	<view class="memberainer">
		...
		<view class="memberBg">
			<view class="memberOver" v-for="(item,index) in memberList" :key="index" @click="godetail(item.member_id)" >
				<view class="idxMainBox">
					<view class="memberOverLt">
						<view class="memberName">{{item.member_name}}</view>
						<view class="memberBuytime">注册日期：{{item.member_regitime}}</view>
						<view class="membermember">最近购买：{{item.member_lastbuytime}}</view>	
						<view class="membermachnum">账户余额：{{item.member_wealth}}元</view>					</view>
					<view class="memberOverRt">
						<view class="memberPrice">{{item.member_buycount}}次</view>
						<view class="memberMore">查看更多>></view>
					</view>
				</view>
			</view>
		</view>
...
<style>
	...
	.memberOver{ border-bottom: 1px #EDEDED solid;overflow: hidden;transition: all 0.4s; }
	.memberOver:active{ background: #E2E2E2; }
	.memberOver:last-child{ border-bottom: none; }
	.memberOverLt{ float: left;width: 60%; }
	.memberName{
		color: #333333;
		font-size: 30upx;
		width: 100%;
	...
	.memberBuytime,.membermember,.membermachnum{ color: #888888;font-size: 24upx;margin-top: 10upx; }
	.memberMore{ color: #888888;font-size: 24upx;margin-top: 30upx; }
	.memberOverRt{ float: right;width: 35%;height: 155upx;margin: 30upx 0; }
	.memberPrice{
		margin-top: 30upx;
		margin-left: 40%;
	}
